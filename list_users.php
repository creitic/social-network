...
$q=$db->query("SELECT id,name,pseudo,email,avatar FROM users WHERE active='1' ORDER BY pseudo $limit");
...
if ($last_page!=1) {
	if ($page_num>1) {
		$previous=$page_num-1;
		$pagination .='<li><a href="list_users.php?page='.$previous.'" aria-label="Precedent"><span aria-hidden="true">&laquo;</span></a></li>';
...
require('views/list_users.view.php');
	...
	set_flash('Aucun utilisateurs trouvé!');
	redirect('index.php');
}
